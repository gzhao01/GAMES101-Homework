...
{   
    ...
    Eigen::Vector2f AB = B-A, BC = C-B, CA = A-C;
    Eigen::Vector2f AP = P-A, BP = P-B, CP = P-C;
    // std::cout<<AB[0]*AP[1]-AB[1]*AP[0]<<"\n";
    // std::cout<<BC[0]*BP[1]-BC[1]*BP[0]<<"\n";
    // std::cout<<CA[0]*CP[1]-CA[1]*CP[0]<<"\n";
...
void rst::rasterizer::draw(pos_buf_id pos_buffer, ind_buf_id ind_buffer, col_buf_id col_buffer, Primitive type)
{
    ...
    for (auto& i : ind)
    {
        ...
        for (auto & vert : v)
        {
            ...
            vert.z() = -vert.z() * f1 + f2; //origin frame has problem
        }
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    //change properties v from vector3 to vector4
    auto v = t.toVector4();
    ...
    if(superSampling){
        ...
        for(int i=min_x;i<=max_x;i++){
            for(int j=min_y;j<=max_y;j++){
                ...
                int count = 0; //count the pixel lay in triangle
                for(int k=0;k<=3;k++){
                    if(insideTriangle((float)i+pos[k][0], (float)j+pos[k][1], t.v)){
                        //get interploted depth
                        auto tup = computeBarycentric2D((float)i+pos[k][0], (float)j+pos[k][1], t.v);
